...
import { ChevronDown, Settings, FileText, RefreshCw, Upload } from 'lucide-react';
...
import { generateSurnamesApi } from "@/apis/api";
...
export default function LiveVoterListImportExportPage() {
  ...
  const handleGenerateFamilyIds = async () => {
    ...
    if (!confirm(`This will generate family IDs for selected data IDs: ${selectedDataIds.join(', ')}. Continue?`)) {
      return;
    }
    try {
      ...
      alert(`Error generating family IDs: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const handleCastIdOnFamily = async () => {
    ...
    if (!confirm(`This will apply cast ID on family members for selected data IDs: ${selectedDataIds.join(', ')}. Continue?`)) {
      return;
    }
    try {
      ...
      alert(`Error applying cast ID on family: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const { dataId: sharedDataId, setDataId: setSharedDataId } = useSharedDataId();
  ...
  const [familyIdResult, setFamilyIdResult] = useState<FamilyIdResponse | null>(null);
  ...
  const isDisabled = generatingFamilyIds ? generatingFamilyIds || selectedDataIds.length === 0 : generatingFamilySurname || selectedDataIds.length === 0;
  const tooltip = selectedDataIds.length === 0
    ...
    return (
      <div className="bg-gray-900 text-white p-4 rounded font-mono text-sm">
        <div className="flex justify-between items-center mb-2">
          ...
          <button onClick={handleClear} className="text-xs text-gray-400 hover:text-white">Clear</button>
        </div>
        <div
          ref={outputRef}
  ...
  const handleCastIdBySurnameOnFamily = async () => {
    ...
    if (!confirm(`This will apply cast ID by surname on family members for selected data IDs: ${selectedDataIds.join(', ')}. This may take some time. Continue?`)) {
      return;
    }
    ...
    try {
      ...
      if (typeof (apiService as any).castIdBySurnameOnFamily === 'function') {
        result = await (apiService as any).castIdBySurnameOnFamily(selectedDataIds);
      } else {
        // fallback: simulate success
        result = {
          success: true,
      ...
      alert(`Error applying cast ID by surname on family: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const handleGenerateSurnames = async () => {
    ...
    if (!confirm(`This will generate surnames for selected data IDs: ${selectedDataIds.join(', ')}. Continue?`)) {
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        alert(`Success! Surnames generated for ${selectedDataIds.length} data IDs.`);
      } else {
        alert(`Error: ${result.message}`);
      }
    } catch (error) {
      console.error('Error generating surnames:', error);
      alert(`Error generating surnames: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto bg-white">
        ...
        ${isDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-gray-600 text-white hover:bg-gray-700 cursor-pointer"}`}
                    title={tooltip}
                  >
                    <RefreshCw size={16} className={generatingFamilySurname ? "animate-spin" : ""} />
                    <span>{generatingFamilySurname ? "Generating..." : "Generate Surnames"}</span>
                  </button>
                  <button
                    onClick={handleGenerateFamilyIds}
                    disabled={generatingFamilyIds || selectedDataIds.length === 0}
                    ...
                    title={selectedDataIds.length === 0 ? 'Please select at least one row from the import sessions table' : 'Generate family IDs for selected rows'}
                  >
                    <RefreshCw size={16} className={generatingFamilyIds ? 'animate-spin' : ''} />
                    <span>{generatingFamilyIds ? 'Generating...' : 'Generate Family IDs'}</span>
                  </button>
                  <button
                    onClick={handleCastIdOnFamily}
                    disabled={applyingCastId || selectedDataIds.length === 0}
                    className="flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 transition-colors duration-200 font-medium text-sm cursor-pointer disabled:bg-gray-400 disabled:cursor-not-allowed"
                    title={selectedDataIds.length === 0 ? 'Please select at least one row from the import sessions table' : 'Apply cast ID on family for selected rows'}
                  >
                    <RefreshCw size={16} className={applyingCastId ? 'animate-spin' : ''} />
                    <span>{applyingCastId ? 'Processing...' : 'Cast Id on Family'}</span>
                  </button>
                  <button
                    onClick={handleCastIdBySurnameOnFamily}
                    disabled={applyingCastIdBySurname || selectedDataIds.length === 0}
                    className="flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 transition-colors duration-200 font-medium text-sm cursor-pointer disabled:bg-gray-400 disabled:cursor-not-allowed"
                    title={selectedDataIds.length === 0 ? 'Please select at least one row from the import sessions table' : 'Apply cast ID by surname on family for selected rows'}
                  >
                    <RefreshCw size={16} className={applyingCastIdBySurname ? 'animate-spin' : ''} />
                    <span>{applyingCastIdBySurname ? 'Processing...' : 'Cast Id by Surname on Family'}</span>
                  </button>

                  <button
                    onClick={() => {
                      setImportRowClicked(true)
                      const el = document.getElementById('import-add-button') as HTMLButtonElement | null;
                      if (el) {
                        el.click();
              ...
              {familyIdResult && (
                <div className={`mt-4 p-3 rounded ${familyIdResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border-red-200'}`}>
                  <p className={`text-sm ${familyIdResult.success ? 'text-green-800' : 'text-red-800'}`}>
                  ...
                  {familyIdResult.success && (
                    <div className="mt-2 text-xs text-green-700">
                      ...
                      <p>Family IDs Generated: {familyIdResult.familyIdsGenerated}</p>
              ...
              {castIdResult && (
                <div className={`mt-4 p-3 rounded ${castIdResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border-red-200'}`}>
                  <p className={`text-sm ${castIdResult.success ? 'text-green-800' : 'text-red-800'}`}>
