...
interface ModuleAssignmentModalProps {
  ...
  setDirtyFields: (
    fields: Set<string> | ((prev: Set<string>) => Set<string>)
  ) => void;
...
export default function ModuleAssignmentModal({
  ...
  const [expandedNavbarSections, setExpandedNavbarSections] = useState<
    Set<string>
  >(new Set());
  const [modulesFromDb, setModulesFromDb] = useState<
    Array<{ id: number; module_id: string; module_name: string }>
  >([]);
  const [datasetsFromDb, setDatasetsFromDb] = useState<
    Array<{ id: number; dataset_id: string; dataset_name: string }>
  >([]);
    ...
    if (!selectedItemId && modulesFromDb.length > 0) {
      ...
      if (assigned.length > 0) {
        ...
        const firstAvailable =
          modulesFromDb.find((m) => assigned.includes(m.module_id))
            ?.module_id ||
          datasetsFromDb.find((d) => assigned.includes(d.dataset_id))
            ?.dataset_id;
  ...
  const getDatasetMeta = (datasetId: string) => {
    ...
    return {
      icon: isOld ? "📄" : "📊",
      title:
        datasetsFromDb.find((d) => d.dataset_id === datasetId)?.dataset_name ||
        datasetId,
      description: isOld
  ...
  const handlePermissionToggle = (itemId: string, permission: string) => {
    ...
    const [moduleId, menuId] = itemId.includes(":")
      ? itemId.split(":")
      : [itemId, null];
    ...
    setEditedUser((prev: any) =>
      prev ? { ...prev, permissions: newPermissions } : prev
    );
  ...
  const handlePasteConfig = () => {
    try {
      ...
      setPasteError("");
      alert(
        "Configuration applied successfully! Click Save Changes to persist."
      );
    } catch (err) {
                  ...
                  <div className="relative group flex items-center">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const u = users.find(
                          (user) => String(user.id) === String(uid)
                        );
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-1/2 border-r border-gray-100 flex flex-col bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              {/* Modules Section */}
              <div>
                <h5 className="text-xs font-semibold text-gray-400 mb-3 uppercase tracking-tighter">
                  Application Modules
                </h5>
                <div className="space-y-1">
                  {modulesFromDb.map((m) => {
                    ...
                    const isAssigned =
                      editedUser?.assignedModules?.includes(m.module_id) ||
                      false;
                            ...
                            onChange={(e) => {
                              ...
                              const newModules = e.target.checked
                                ...
                                : currentModules.filter(
                                    (id: string) => id !== m.module_id
                                  );
                              setEditedUser((prev: any) =>
                                prev
                                  ? { ...prev, assignedModules: newModules }
                                  : prev
                              );
                              setDirtyFields(
                                (prev: Set<string>) =>
                                  new Set([...prev, "assignedModules"])
                              );
                            ...
                            <div className="ml-10 mt-2 space-y-1 border-l-2 border-blue-200 pl-3">
                              {meta.menus.map((menu) => {
                                ...
                                const isMenuSelected =
                                  editedUser?.assignedNavbarPages?.includes(
                                    menuPageId
                                  ) || false;
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setEditedUser((prev: any) => {
                                          if (!prev) return prev;
                                          const currentPages =
                                            prev.assignedNavbarPages || [];
                                          const newPages = isMenuSelected
                                            ? currentPages.filter(
                                                (p: string) => p !== menuPageId
                                              )
                                            : [...currentPages, menuPageId];
                                          return {
                                            ...prev,
                                            assignedNavbarPages: newPages,
                                          };
                                        });
                                      }}
              ...
              <div>
                ...
                <div className="space-y-1">
                  {datasetsFromDb.map((dataset) => {
                    ...
                    const isAssigned =
                      editedUser?.assignedModules?.includes(
                        dataset.dataset_id
                      ) || false;
                    ...
                    return (
                      <>
                        <div
                          ...
                          onClick={() =>
                            !datasetMeta.isDisabled &&
                            setSelectedItemId(dataset.dataset_id)
                          }
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              const newModules = e.target.checked
                                ...
                                : currentModules.filter(
                                    (id: string) => id !== dataset.dataset_id
                                  );
                              setEditedUser((prev: any) =>
                                prev
                                  ? { ...prev, assignedModules: newModules }
                  ...
                  <h4 className="text-sm font-bold text-gray-600 uppercase tracking-wider flex items-center gap-2">
                    ...
                    {(() => {
                      ...
                      if (selectedItemId.includes(":")) {
                        ...
                        const moduleFromDb = modulesFromDb.find(
                          (m) => m.module_id === moduleId
                        );
                        const datasetFromDb = datasetsFromDb.find(
                          (d) => d.dataset_id === moduleId
                        );
                        const moduleMeta =
                          getModuleMeta(moduleId) || getDatasetMeta(moduleId);
                        const menu = moduleMeta?.menus?.find(
                          (m: any) => m.id === menuId
                        );
                        const name =
                          moduleFromDb?.module_name ||
                          datasetFromDb?.dataset_name ||
                          moduleId;
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  <div className="grid grid-cols-2 gap-3">
                    {getPermissionsForSelected().map((permission) => {
                      ...
                      const [moduleId, menuId] = selectedItemId.includes(":")
                        ? selectedItemId.split(":")
                        : [selectedItemId, null];
                      const prefix =
                        MODULE_TO_PERMISSION_PREFIX[moduleId] || moduleId;
                      ...
                      const isChecked =
                        editedUser?.permissions?.some((p: string) => {
                      ...
                      return (
                        <label
                          key={permission}
                          className={`flex items-start p-3 rounded-lg border transition-all cursor-pointer h-full ${
                            isChecked
                        ...
                        >
                          <input
                            ...
                            checked={isChecked}
                            onChange={() =>
                              handlePermissionToggle(selectedItemId, permission)
                            }
                            className="mt-0.5 h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500 shrink-0"
                          />
                          <div className="ml-3 flex-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-sm font-semibold text-gray-800 capitalize leading-tight">
                    ...
                    {getPermissionsForSelected().length === 0 && (
                      <div className="flex flex-col items-center justify-center py-12 text-center text-gray-400">
                        ...
                        <p className="text-xs">
                          Standard access is controlled by the module assignment
                          on the left.
                        </p>
                      </div>
                    )}
