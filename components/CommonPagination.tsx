...
interface PaginationProps {
  ...
  // Count of rows actually rendered on the current page (data.length)
...
export default function CommonPagination({
  ...
  const computedTotalPages = typeof numericTotalItems === 'number'
    ? (itemsPerPage === 'All' ? 1 : Math.max(1, Math.ceil(numericTotalItems / Number(itemsPerPage))))
    : totalPages;
  ...
  const handleItemsPerPageSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (val === 'All') {
      ...
      if (currentPage !== 1) setGoToPageInput('1');
      return;
    }
  ...
  return (
    <div
      className="bg-white px-2 py-1 border-t border-gray-200 flex-shrink-0 w-full z-30"
    >
      <div className="flex flex-col sm:flex-row justify-between items-center text-sm text-gray-700 space-y-1 sm:space-y-0">
        <div className="text-center sm:text-left">
          {loading ? (
            ...
            <div className="text-sm">
              ...
              <span>Showing {(() => {
                const pageSize = itemsPerPage === 'All' ? numericTotalItems || 1 : Number(itemsPerPage);
                const startRow = (currentPage - 1) * pageSize + 1;
                // Prefer the actual number of rows rendered on this page when available
                const countOnPage = typeof currentPageItemCount === 'number' && currentPageItemCount >= 0
                  ? currentPageItemCount
                  : pageSize;
                const proposedEnd = Math.max(startRow - 1, startRow + countOnPage - 1);
                const endRow = typeof numericTotalItems === 'number'
                  ? Math.min(proposedEnd, numericTotalItems)
                  : proposedEnd;
                return `${startRow} to ${endRow}`;
              })()} entries</span>
              {itemsPerPage !== 'All' && ![50, 100, 500, 1000, 2000, 5000].includes(Number(itemsPerPage)) && (
